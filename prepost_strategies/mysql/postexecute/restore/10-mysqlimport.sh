...
function databaseJob() {
  ...
  pv ${MYSQL_SOURCE}/dump-${VOLUMERIZE_DB_DATABASE}.sql | mysql -h ${VOLUMERIZE_DB_HOST} -u ${VOLUMERIZE_DB_USERNAME} -p${VOLUMERIZE_DB_PASSWORD} ${VOLUMERIZE_DB_EXTRA_ARGS} ${VOLUMERIZE_DB_DATABASE} || returnCode=$? && true ;
  echo "Import done"
