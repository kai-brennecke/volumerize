...
This fork was created in an attempt to keep dependencies up to date. This will primarily be accomplished using dependabot.
...
- [Volumerize](#volumerize)
  ...
  - [Customize Supercronic](#customize-supercronic)
...
The default cron setting for this container is: `0 0 4 * * * *`. That's four o'clock in the morning UTC. You can set your own schedule with the environment variable `VOLUMERIZE_CRON_TIME`.

You can set the time zone with the environment variable `TZ`.

The syntax is different from cron because I use Supercronic as a cron tool: [Supercronic documentation](https://github.com/aptible/supercronic)
...
## Customize Supercronic


If you want to customize supercronic more with different commands, you should write your own crontab file and mount it at a custom location specified in `VOLUMERIZE_CRON_CUSTOM` (will be copied to `/root/volumerize-crontab`). More information on how to write supercronic crontabs can be found [here](https://github.com/aptible/supercronic). For example the file generated by the automated scripts could look like this:
...
0 0 4 * * * * /etc/volumerize/periodicBackup 1


# VolumerizeBackupJob2

0 0 4 * * * * /etc/volumerize/periodicBackup 2
```
